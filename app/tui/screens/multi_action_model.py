...
class MultiActionModal(ModalScreen):
    ...
    def on_mount(self):
        if not self.action:
            self.notify("No tools support bulk action", severity='error')
            self.dismiss()
        self._render_list()
        self._update_preview()
        self.query_one("#action-input", Input).display = False
        self.focus()
    ...
    def on_key(self, event):
        ...
        if event.key in ['down', 'up', 'right', 'left']:
            if event.key == 'down':
    ...
    def _update_preview(self):
        ...
        if self.current_index < len(self.action):
            ...
            preview_panel = Panel(
                ...
                title="[bold #FFD700]Bulk Command[/]",
                border_style="#565F89",
                padding=(1, 2)
            )
            self.query_one(
    ...
    def _execute_action(self):
        if self.current_index < len(self.action):
            ...
            success = launch_terminal_multi(key, self.target)
